...
def find_pii_and_doi(text):
    """Searches for PII and DOI in the given text using regex patterns."""
    patterns = {
        "Phone Number": r"\b\d{3}[-.]\d{3}[-.]\d{4}\b|\b\d{3}[-.]\d{4}\b",
        "Birthdate": r"\b(?:0[1-9]|1[0-2])[-/.](?:0[1-9]|[12][0-9]|3[01])[-/.](?:19|20)\d{2}\b",
        "Driver's License": r"\b(?:DL|Driver'?s License)(?:\s+\w+){0,4}\s*([A-Z0-9-]{6,10}\d[A-Z0-9-]*)\b(?![-.\s]?\d{3}[-.\s]?\d{3}[-.\s]?\d{4})",
...
def scan_folder(input_folder, output_csv):
    """Scans PDFs and image files in a folder for PII and DOIs, saving results to a CSV."""
    results = []
    ...
    with open(output_csv, mode="w", newline="", encoding="utf-8") as file:
        ...
        writer.writerow(["Document Name", "Detected Information"])
        writer.writerows(results)
    
    print(f"Scan complete. Results saved to {output_csv}")

if __name__ == "__main__":
    scan_folder("A", "B/pii_doi_report.csv")
